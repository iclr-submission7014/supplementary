(async()=>{
 ...
 try{
  ...
  root.append(el('h2','section-title','2.4 Typography with Spatial Token Control'));
  ...
  root.append(el('p','type-note','Red cells are finest; blue cells are coarsest. Colors encode token area, including rectangular tokens. Click any image to enlarge the comparison.'));
  ...
  function statistics(e,node){node.replaceChildren();const speed=el('div','type-stat'),compression=el('div','type-stat');speed.append(el('strong','',`${e.seconds.speedup.toFixed(2)}× speedup`),el('span','',`Ours ${e.seconds.ours.toFixed(1)} s · Dense ${e.seconds.dense.toFixed(1)} s`));compression.append(el('strong','',`${(e.dense_tokens/e.tokens).toFixed(2)}× token compression`),el('span','',`${e.tokens.toLocaleString()} / ${e.dense_tokens.toLocaleString()} tokens (ours / full resolution)`));node.append(speed,compression)}
